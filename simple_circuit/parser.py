    ...
    for node_id, node in nodes.items():
        ...
        if node_type in ['default']:
            ...
            if len(input_sources) >= 2:
                ...
                gates.append(Gate(gate_type, first, second))
            protein = Protein(current_protein_id, label, init_conc, hill, degrad, gates)
            node_to_protein_idx[node_id] = current_protein_id
            protein_array.append(protein)
            current_protein_id += 1
            ...
            continue  # Outputs are not proteins
...
if __name__ == "__main__":
    ...
    for p in proteins:
        print(f"Node ID: {p.mID}, Name: {p.mName}, Gates: {[ (g.mType, g.mFirstInput, g.mSecondInput) for g in p.mGates ]}")
