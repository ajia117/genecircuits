...
# DEFINE THE SLIDERS
n_x_slider = pn.widgets.FloatSlider(name='Hill coefficient n_x', start=0.5, end=5, step=0.1, value=1)
n_y_slider = pn.widgets.FloatSlider(name='Hill coefficient n_y', start=1, end=10, step=0.1, value=4)
duty_cycle_slider = pn.widgets.FloatSlider(name='Duty cycle', start=0.1, end=1.0, step=0.1, value=0.2)

colors = bokeh.palettes.d3['Category10'][10]

@pn.depends(n_x_slider.param.value, n_y_slider.param.value, duty_cycle_slider.param.value)
def update_plot(n_x, n_y, duty_cycle):
    # Set up parameters for the pulse
    x_args = (3.0, 19.0, 5.0, 2.0, duty_cycle)
    ...
    # Create plots
    p = bp.figure(width=500, height=300, x_axis_label='dimensionless time', y_axis_label='dimensionless y, z')
    p.line(t, y, line_width=2, color=colors[1], legend_label='y')
    p.line(t, z, line_width=2, color=colors[2], legend_label='z')
    p.legend.location = 'bottom_right'

    p_x = bp.figure(width=500, height=300, x_axis_label='dimensionless time', y_axis_label='dimensionless x')
    p_x.line(t, x_pulse(t, 3.0, 19.0, 5.0, 2.0, duty_cycle), line_width=2)

    return pn.Column(p, p_x)


# Set up widgets and interactive plot
widgets = pn.Column(n_x_slider, n_y_slider, duty_cycle_slider)
interactive_plot = pn.Row(widgets, update_plot)

# Make the interactive plot servable
interactive_plot.servable()

# Launch the Panel app (no need for output_file or bokeh.io.show)
pn.serve(interactive_plot)

# RUN IN TERMAL USING: panel serve your_script_name.py
...
#Show plot
output_file("plots/ffl_long_pulse_with_sliders_plot.html")
